...
class Home_model extends CI_Model{
    ...
    function getAllCountAdminReportGraph($monthYear){
        $dateSplit=explode('/',$monthYear);
        $this->db->select("count(userID) AS cnt,day(createdAt) createdDay");
        $this->db->from("userMaster");
        $this->db->where("MONTH(createdAt)=$dateSplit[0] AND YEAR(createdAt)=$dateSplit[1]");
        $this->db->group_by('DATE(createdAt)');
        $this->db->order_by('DATE(createdAt)');
        $result = $this->db->get();
