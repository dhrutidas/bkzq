...
class Question_model extends CI_Model{
    ...
    function insertQuestionAns($data)
    {
		...
		$tagging = 'INSERT INTO `questionbankboardmapping`(`qbID`, `levelID`, `stageID`, `boardID`, `subjectID`, `stdID`) VALUES ';
		$stage=1;
		foreach($data['levelID'] as $level){
			foreach($data['board'] as $board){
				foreach($data['subject'] as $subject){
					foreach($data['standard'] as $standard){
						$tagging.="(".$questionID.",'".$level."','".$stage."','".$board."','".$subject."','".$standard."') ,";
					}			
				}		
			}	
		}
