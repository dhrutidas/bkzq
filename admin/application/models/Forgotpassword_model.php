...
class Forgotpassword_model extends CI_Model{
    ...
    function account_check(){
        ...
        $whrArr = array('emailID' => $this->content['post_username'], 'status' => 'Y');
        $this->db->select('*');
    ...
    function updateLoginDetails($dID,$password){
        
        $ip = $_SERVER['REMOTE_ADDR'];
        $now = date('Y-m-d H:i:s');

        $this->db->where('emailID', $dID);
        $this->db->update('userMaster', array('password' => MD5($password), 'lastLoginIp' => $ip, 'lastLoginTime' => $now));
        return ( $this->db->affected_rows() === 1 ) ? TRUE : FALSE;
    }
}